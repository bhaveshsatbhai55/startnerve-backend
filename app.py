...
def has_credits(uid, engine_type):
    ...
    if not credits: return False
    return credits.get(engine_type, 0) > 0
...
def create_user_endpoint():
    """Creates a user document in Firestore on first sign-up."""
    try:
        data = request.get_json()
        uid = data.get('uid')
        print(f"--- ATTEMPTING TO CREATE USER WITH UID: {uid} ---") # ADDED FOR DEBUGGING
        if not uid:
        ...
        user_ref = db.collection('users').document(uid)
        user_ref.set(DEFAULT_CREDITS, merge=True) 
        return jsonify({"status": "success", "message": "User created with default credits."}), 201
    except Exception:
        traceback.print_exc()
...
def generate_outline_endpoint():
    try:
        data = request.get_json()
        uid = data.get('uid') 
        print(f"--- EBOOK OUTLINE RECEIVED UID: {uid} ---") # ADDED FOR DEBUGGING
...
def process_lesson(lesson_args):
    ...
    # YOUR NEW FEATURE: Clean duplicate paragraphs
...
def generate_text_content_route():
    try:
        data = request.get_json()
        uid = data.get('uid')
        print(f"--- EBOOK CONTENT RECEIVED UID: {uid} ---") # ADDED FOR DEBUGGING
        if not uid: return jsonify({"error": "User not authenticated"}), 401
        if not has_credits(uid, "ebook"):
            return jsonify({"error": "You're out of ebook credits!"}), 403
...
def generate_full_ebook_route():
    try:
        data = request.get_json()
        uid = data.get('uid')
        print(f"--- EBOOK EXPORT RECEIVED UID: {uid} ---") # ADDED FOR DEBUGGING
        if not uid: return jsonify({"error": "User not authenticated"}), 401
        if not has_credits(uid, "ebook"):
            return jsonify({"error": "You're out of ebook credits!"}), 403
...
def generate_viral_content_endpoint():
    try:
        data = request.get_json()
        uid = data.get('uid')
        print(f"--- VIRAL ENGINE RECEIVED UID: {uid} ---") # ADDED FOR DEBUGGING
        if not uid: return jsonify({"error": "User not authenticated"}), 401
